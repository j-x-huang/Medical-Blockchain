...
async function updatePatientEncounter(MedicalEncounter) {
    ...
    MedicalEncounter.patient.record.push(MedicalEncounter.record);
...
/**
 * Remove a patient
 * @param {nz.ac.auckland.RemovePatient} remove - the remove to be processed
 * @transaction
 */
async function removePatient(remove) {
    ...
    let results = await query('selectPatientByID', { pid: remove.patient.pid });
